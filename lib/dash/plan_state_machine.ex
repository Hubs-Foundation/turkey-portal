defmodule Dash.PlanStateMachine do
  ...
  @business_ccu_limit 400
  @business_storage_limit_mb 200_000
  ...
  def handle_event(
    ...
    %{plan_id: plan_id} = Repo.insert!(%Plan{account_id: account.account_id})
    :ok = subscribe_business(plan_id, subscribed_at)

    hub =
      Repo.insert!(%Hub{
        account_id: account.account_id,
        ccu_limit: @business_ccu_limit,
        status: :creating,
        storage_limit_mb: @business_storage_limit_mb,
        subdomain: rand_string(10),
        tier: :b2
      })
  ...
  # CHANGES SUBSCRIPTION TO PERSONAL 
  def handle_event(
        :business,
        {:subscribe_personal, subscribed_at},
        %Account{account_id: account_id, email: email},
  ...
  # ATEMPT TO SUBSCRIPT TO BUSINESS AGAIN 
  def handle_event(
        :business,
        {:subscribe_professional, %DateTime{} = subscribed_at},
        %Account{account_id: account_id},
  ...
  # ALREADY STARTED 
  def handle_event(:business, :start, %Account{}, _data),
    do: {:error, :already_started}

  # FETCH ACTIVE PLAN 
  def handle_event(:business, :fetch_active_plan, %Account{account_id: account_id}, _data),
    do: {:ok, %{fetch_plan!(account_id) | name: "business", subscription?: true}}
  ...
  @spec subscribe_business(Plan.id(), DateTime.t()) :: :ok
  defp subscribe_business(plan_id, %DateTime{} = subscribed_at) when is_integer(plan_id) do
    Repo.insert!(%__MODULE__.PlanTransition{
      event: "subscribe_business",
      new_state: :business,
      plan_id: plan_id,
      transitioned_at: subscribed_at
    })
